//! Backlink path extraction + write preparation helpers for `repo.*` routes.
//!
//! Port of `olamaelcu/rsky` `src/repo/prepare.rs`. `blobs_for_write` returns
//! unconstrained `PreparedBlobRef`s (cacos does not port the app.bsky
//! `lexicons.toml` size/mime constraint table — that lookup is deferred).
