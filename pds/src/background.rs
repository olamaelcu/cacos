...
//! queue to drain. We use it for blob deref cleanup and other async work
//! that must finish eventually but should not block the request that
//! triggered it.
//!
...
mod tests {
    ...
    #[tokio::test]
    async fn pending_reports_outstanding_count() {
        let queue = BackgroundQueue::new(2);
