...
use operations::update_handle_op;
use rsky_common::encode_uri_component;
use secp256k1::SecretKey;
use serde::de::DeserializeOwned;
use types::{CompatibleOp, CompatibleOpOrTombstone, DocumentData, OpOrTombstone};
use url::Url;
...
/// issued. Use [`plc_client_from_env`] to construct one from a PLC URL.
...
impl PlcClient for MockPlcClient {
    ...
    async fn get_last_op(&self, _did: &str) -> Result<CompatibleOpOrTombstone> {
        use operations::CreateAtprotoOpInput;
        use operations::create_op;
