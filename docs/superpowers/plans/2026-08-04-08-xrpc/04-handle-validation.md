# Task 4: Handle validation module port (`crate::handle`)

Needed by createAccount (Task 7), updateHandle (Task 21), and admin updateAccountHandle (Task 25). Port of `the git-pinned `olamaelcu/rsky` fork at rev `aee5aec5ad9473d80232beab58ddba25a936298a` (`rsky` crate's `src/handle/` with the slur regex list simplified to the `rsky_common::explicit_slurs` check.

**Files:**
- Create: `pds/src/handle/mod.rs`
- Create: `pds/src/handle/errors.rs`
- Test: in `pds/src/handle/mod.rs` (in-file unit tests)

- [ ] **Step 1: Write the failing tests**

```rust
// appended to pds/src/handle/mod.rs
#[cfg(test)]
mod tests {
    use super::*;
    ...
    async fn validates_service_domain_handles() {
        // service domain validation does not need the network
...
Run: `cargo test -p pds handle::tests`
Expected: FAIL — module missing.

- [ ] **Step 2: Implement `handle/errors.rs` and `handle/mod.rs`**

```rust
// pds/src/handle/errors.rs
use thiserror::Error;
...
> **Adaptation note:** the reference resolves external handles against the IdResolver in `normalize_and_validate_handle`. In this port, non-service-domain handles are rejected with `UnsupportedDomain` (PDS-ownable scope); the resolver-dependent branch is documented but not implemented. If a deployment needs external handles, add the resolver call here.
...
```bash
git add pds/src/handle
git commit -m "feat(handle): port handle validation for service domains"
```
