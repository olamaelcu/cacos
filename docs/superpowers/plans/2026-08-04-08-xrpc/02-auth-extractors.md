# Task 2: Auth Extractors — `xrpc/auth_extractors.rs`

**Files:**
- Create: `pds/src/xrpc/auth_extractors.rs`
- Test: `pds/src/xrpc/auth_extractors.rs` (unit tests in-file, using Task 1's `test_utils`)

- [ ] **Step 1: Write the failing tests**

```rust
// appended to pds/src/xrpc/auth_extractors.rs
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[handler]
    async fn full_only(auth: AccessFull) -> &'static str {
        "ok"
    }

    #[handler]
    async fn needs_refresh(auth: Refresh) -> &'static str {
        "ok"
    }
...
Run: `cargo test -p pds auth_extractors`
Expected: FAIL — `auth_extractors` module and types do not exist.

- [ ] **Step 2: Implement the extractors**

Each extractor reads the `Authorization` header from the poem `Request`, calls the Plan 06 pure verifier, and converts `AuthError` → `ApiError` → `poem::Error` (via the `ApiError: IntoResponse` impl from Task 1). The scope matrix mirrors `the git-pinned `olamaelcu/rsky` fork at rev `aee5aec5ad9473d80232beab58ddba25a936298a` (`rsky` crate's `src/auth_verifier.rs` exactly.

```rust
// pds/src/xrpc/auth_extractors.rs
use crate::auth::auth_verifier::{
    self, AccessOutput, AuthError, AuthScope, Credentials, DpopRequestContext,
...
/// Maps an auth failure to the wire-facing ApiError exactly as rsky does:
/// ExpiredToken stays distinct, everything else renders as InvalidRequest.
...
/// Plan 06 contract: the pure `validate_access_token` reads the DPoP request
/// context from a thread-local set by the poem layer. Wrap every access-token
/// validation with set/clear scoped to this request. (The absolute-URI form
/// required by RFC 9449 §4.3 can be completed in the OAuth sweep by prefixing
/// `req.uri()` with `config.service.public_url` via `req.data::<SharedState>()`.)
...
> **Plan 06 contract (verified):** Plan 06 (`pds/src/auth/auth_verifier.rs`) exposes exactly the functions above — `validate_access_token(auth_header: Option<&str>, scopes, opts) -> Result<AccessOutput, AuthError>`, `validate_refresh_token(auth_header) -> Result<ValidatedBearer, AuthError>` (fields `did, scope, token, payload, audience`, `payload.jti: Option<String>`), `verify_admin_token`, `verify_user_did_token`, `is_user_or_admin(&AccessOutput, &str)`, `DpopRequestContext { method, uri, dpop_headers }`, `set_dpop_request_context`/`clear_dpop_request_context`, `AuthScope`, `Credentials`, `AccessOutput`, `AuthError` (thiserror variants identical to rsky). Its tests sign tokens with `PDS_JWT_KEY_K256_PRIVATE_KEY_HEX` + `aud = PDS_SERVICE_DID`, matching the `test_utils::init_env` defaults in Task 1. Startup must call `register_auth_dependencies(account_manager, provider)` (add to `SharedStateFromEnv::from_env`) and `register_signing_key_resolver` once IdResolver exists (Plan 06 Task 4 wires this). `Moderator`'s bearer branch is completed once a mod service is configured; admin basic-auth is the tested path.
...
```bash
git add pds/src/xrpc/auth_extractors.rs
git commit -m "feat(xrpc): poem auth extractors wrapping auth_verifier"
```
